...
class AppConfig (filename: String, context: Context){
    ...
    var features: MutableList<FeatureConfig> = ArrayList()
    ...
    private fun initFeatures(featuresArray: MutableList<Any>) {
        ...
        // We want to keep only the features that specify platform as Android or don't specify platform at all
        features = features.filter({ it ->
            (it.platform.isNullOrBlank() || it.platform == "android")
        }) as MutableList<FeatureConfig>
    }
