...
def merge_xml (xmlfile1, xmlfile2, outputfile):
	...
	# Sums up root element
	coverage1 = xml1.getroot()
	coverage2 = xml2.getroot()

	coverage1.set('lines-covered', str(int(coverage1.get('lines-covered', 0)) + int(coverage2.get('lines-covered', 0))))
	coverage1.set('lines-valid', str(int(coverage1.get('lines-valid', 0)) + int(coverage2.get('lines-valid', 0))))

	valid_lines = float(coverage1.get('lines-valid', 0))
	if valid_lines > 0:
		line_rate = float(coverage1.get('lines-covered', 0)) / valid_lines
		coverage1.set('line-rate', str(line_rate))
